...
class OrderController extends Controller
{
    ...
    /**
     * Generates the PDF for the customer order.
     */
    ...
    /**
     * Converts a customer order into supplier orders.
     */
    public function convertToSupplierOrders(Order $order)
    {
        if ($order->status !== 'closed') {
            return redirect()->route('orders.edit', $order->id)
                ->with('error', 'You can only convert orders with status "Closed".');
        }
        $order->load(['lines']);
        ...
        foreach ($linesBySupplier as $supplierId => $lines) {
            ...
            $nextId = SupplierOrder::max('id') + 1;
            $number = 'ENF-' . str_pad($nextId, 5, '0', STR_PAD_LEFT);
        ...
        return redirect()->route('supplier-orders.index')
            ->with('success', 'Supplier orders created successfully!');
    }
}
